// from: github.com/jda/bloomfilter/blob/master/bloomfilter.go
...
func NewFromBytes(bb []byte, k int) *BloomFilter {
	...
	for i := range ii {
		ii[i] = binary.LittleEndian.Uint32(bb[i*4 : (i+1)*4])
	}
	return &BloomFilter{
		m:       uint32(len(ii) * 32),
		k:       k,
...
// EstimateParameters estimates requirements for m.
// https://github.com/willf/bloom
func EstimateParameters(n int, p float64) int {
	return int(math.Ceil(-1 * float64(n) * math.Log(p) / math.Pow(math.Log(2), 2)))
}
...
func (bf *BloomFilter) locations(v []byte) []uint32 {
	...
	var x = int32(a) % int32(bf.m)
	for i := range r {
		if x < 0 {
			r[i] = uint32(x + int32(bf.m))
		} else {
			r[i] = uint32(x)
		}
		x = (x + int32(b)) % int32(bf.m)
	}
	return r
}
...
func (bf *BloomFilter) ToBytes() []byte {
	...
	for _, bucket := range bf.buckets {
		var a = make([]byte, 4)
		binary.LittleEndian.PutUint32(a, bucket)
		bb = append(bb, a...)
	}
	return bb
}
